#!/bin/bash
# RED-Feather Plugin Installer (lightweight method)

REPO="StandardCodebase/obsidian-red-signer"
PLUGIN_NAME="red-feather"
...
case "$OS" in
    linux)
        case "$ARCH" in
            x86_64) BIN_NAME="red-feather-linux-x64" ;;
            aarch64) BIN_NAME="red-feather-linux-arm64" ;;
            *) echo "❌ Unsupported architecture: $ARCH"; exit 1 ;;
        esac
        ;;
    darwin)
        case "$ARCH" in
            x86_64) BIN_NAME="red-feather-macos-x64" ;;
            arm64) BIN_NAME="red-feather-macos-arm64" ;;
            *) echo "❌ Unsupported architecture: $ARCH"; exit 1 ;;
        esac
        ;;
    *)
...
echo "2. Enable 'RED-Feather' in Settings → Community plugins"
